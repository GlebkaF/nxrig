...
const enumFromKeys = (obj) => z.custom((val) => typeof val === 'string' && Object.prototype.hasOwnProperty.call(obj, val), {
...
export const CompSchema = z
  .object({ enabled: boolOpt, type: enumFromKeys(COMP_TYPES), sustain: num0100, level: num0100, attack: num0100, blend: num0100 })
  .partial({ sustain: true, level: true, attack: true, blend: true });

export const EfxSchema = z
  .object({ enabled: boolOpt, type: enumFromKeys(EFX_TYPES), var1: num0100, var2: num0100, var3: num0100, var4: num0100, var5: num0100, var6: num0100 })
  .partial({ var1: true, var2: true, var3: true, var4: true, var5: true, var6: true });

export const AmpSchema = z
  .object({ enabled: boolOpt, type: enumFromKeys(AMP_TYPES), gain: num0100, master: num0100, bass: num0100, mid: num0100, treble: num0100, bright: num0100 })
  .partial({ gain: true, master: true, bass: true, mid: true, treble: true, bright: true });

export const CabSchema = z
  .object({ enabled: boolOpt, type: enumFromKeys(CABINET_TYPES), level: num0100, lowcut: num0100, hicut: num0100 })
  .partial({ level: true, lowcut: true, hicut: true });

export const ModSchema = z
  .object({ enabled: boolOpt, type: enumFromKeys(MOD_TYPES), rate: num0100, depth: num0100, mix: num0100 })
  .partial({ rate: true, depth: true, mix: true });

export const DelaySchema = z
  .object({ enabled: boolOpt, type: enumFromKeys(DELAY_TYPES), time: num0100, feedback: num0100, mix: num0100 })
  .partial({ time: true, feedback: true, mix: true });

export const ReverbSchema = z
  .object({ enabled: boolOpt, type: enumFromKeys(REVERB_TYPES), decay: num0100, tone: num0100, mix: num0100 })
  .partial({ decay: true, tone: true, mix: true });

export const EqSchema = z
  .object({ enabled: boolOpt, type: z.enum(['6-Band', '10-Band', 'Bluetooth EQ', 'Speaker EQ']) })
  .catchall(num0100);
