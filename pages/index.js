...
function buildParamsForVisualization(slot, model, section) {
  ...
  if (slot === "RVB") {
    ...
    for (const [k, meta] of Object.entries(cfg.params)) {
      ...
      if (lbl.includes("tone") || lbl.includes("liveliness"))
        add(k, section?.tone);
    }
...
function flatPresetToVisualizationChain(preset) {
  ...
  if (preset.noise_gate)
    chain.push({
      slot: "Noisegate",
      model: "Noise Gate",
      enabled: preset.noise_gate.enabled,
      params: {
        Sens: preset.noise_gate.sensitivity ?? 50,
  ...
  if (preset.comp)
    chain.push({
      slot: "Compressor",
      model: preset.comp.type,
      enabled: preset.comp.enabled,
      params: buildParamsForVisualization(
        "Compressor",
        preset.comp.type,
        preset.comp
      ),
  ...
  if (preset.efx)
    chain.push({
      slot: "EFX",
      model: preset.efx.type,
      enabled: preset.efx.enabled,
      params: buildParamsForVisualization("EFX", preset.efx.type, preset.efx),
    });
  if (preset.amp)
    chain.push({
      slot: "Amp",
      model: preset.amp.type,
      enabled: preset.amp.enabled,
      params: buildParamsForVisualization("Amp", preset.amp.type, preset.amp),
    });
  if (preset.cab)
    chain.push({
      slot: "Cabinet",
      model: preset.cab.type,
      enabled: preset.cab.enabled,
      params: buildParamsForVisualization(
        "Cabinet",
        preset.cab.type,
        preset.cab
  ...
  if (preset.mod)
    chain.push({
      slot: "Mod",
      model: preset.mod.type,
      enabled: preset.mod.enabled,
      params: buildParamsForVisualization("Mod", preset.mod.type, preset.mod),
    });
  if (preset.delay)
    chain.push({
      slot: "Delay",
      model: preset.delay.type,
  ...
  if (preset.reverb)
    chain.push({
      slot: "RVB",
      model: preset.reverb.type,
      enabled: preset.reverb.enabled,
      params: buildParamsForVisualization(
        "RVB",
        preset.reverb.type,
        preset.reverb
  ...
  if (preset.eq)
    chain.push({
      slot: "EQ",
      model: preset.eq.type,
      enabled: preset.eq.enabled,
      params: buildParamsForVisualization("EQ", preset.eq.type, preset.eq),
    });
  return {
    chain: chain
      .filter((b) => b.enabled !== false)
      .sort(
        (a, b) => CHAIN_ORDER.indexOf(a.slot) - CHAIN_ORDER.indexOf(b.slot)
      ),
  };
}
...
export default function Mp3QrPage() {
  const [text, setText] = useState(() =>
    JSON.stringify(randomPreset(), null, 2)
  );
        ...
        <div className="grid md:grid-cols-2 gap-6">
          <div className="bg-gray-800 rounded-2xl p-4 border border-gray-700">
            <div className="flex gap-3 items-center mb-3">
              ...
              <button
                onClick={() => setText(JSON.stringify(randomPreset(), null, 2))}
                className="px-4 py-2 rounded-xl bg-gray-700 hover:bg-gray-600"
              >
                Random
              </button>
            ...
            <textarea
              ...
              placeholder="JSON will appear here after import..."
              value={text}
